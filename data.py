...
def update_email_cadastrado(active_cell, rows):
    if active_cell and active_cell['column_id'] == 'CADASTRADO':
        row = active_cell['row']
        value = rows[row].get('CADASTRADO', "Não Cadastrado")
        rows[row]['CADASTRADO'] = "Cadastrado" if value != "Cadastrado" else "Não Cadastrado"
    return rows
    ...
    if n_clicks and n_clicks > 0:
        ...
        if cadastrados:
            ...
            try:
                ...
                try:
                    historico_df = pd.read_excel(caminho_historico, sheet_name='historico')
                except FileNotFoundError:
                    historico_df = pd.DataFrame()
                for row in cadastrados:
                    row['DataResposta'] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
